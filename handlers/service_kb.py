...
async def handle_service_action(callback_query: CallbackQuery, bot: Bot):
    ...
    await bot.send_message(callback_query.from_user.id, f"{text}", reply_markup=keyboard)

# Перезапуск службы
@router.callback_query(F.data.startswith("restart_"))
async def restart_service(callback_query: CallbackQuery):
    await callback_query.answer()
    service_name = callback_query.data.split("_")[1]
...
async def toggle_service(callback_query: CallbackQuery):
    await callback_query.answer()
    service_name = callback_query.data.split("_")[1]
...
async def edit_service(callback_query: CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.answer(f"Введите новое название для службы {service_name}:")
